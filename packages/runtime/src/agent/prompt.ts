...
export function buildOpencodeSystemPrompt(
  ...
  const codingOutputContext = `\n# Coding run output\nWhen you perform repo-changing coding work, create or update .tags/run-output.json in the changed repo. Use this JSON shape when known: {"prUrl":"https://github.com/owner/repo/pull/123","repoUrl":"https://github.com/owner/repo","branch":"branch-name","commitSha":"sha","demo":{"kind":"web","repoSubdir":"optional/path","installCommand":"optional command","skipInstall":false,"startCommand":"command to run the app","readyUrl":"http://127.0.0.1:3000/path","readyTimeoutMs":45000,"steps":[{"type":"navigate","url":"http://127.0.0.1:3000/path"},{"type":"waitForText","text":"visible fixed UI text"}],"successText":"optional short description"}}. For non-UI work use demo.kind \"terminal\" with a command, or \"none\" with a reason. Do not include secrets.`;

  const demoRecordingContext = options?.demoRecordingRequested
    ? `\n# Demo recording required\nThe user asked for a video, screencast, or visual proof of this change. Before you finish, you MUST create or update .tags/run-output.json in the changed repo with prUrl, repoUrl, branch, commitSha, and a concrete demo recipe invented from the codebase and the files you changed.\n- Prefer demo.kind \"web\" when the change is UI-visible: inspect package scripts and routes, then set startCommand, readyUrl, and a SHORT Playwright step list (prefer navigate + waitForText/assertText; add click/fill only when needed). Do not invent fake URLs or selectors. Keep demos under ~60s of interaction.\n- The demo recorder clones the PR branch (and checks out commitSha when present) into its own sandbox cwd and cds into repoSubdir for you. Use relative commands only (e.g. \"npm install\", \"npx next dev --port 3000\"). Do NOT hardcode /home/user/repo paths.\n- The recorder has Node/npm/corepack (pnpm/yarn via corepack) — not bun. Prefer npm/npx/pnpm. Omit installCommand when a lockfile is present so Tags can infer it. Set skipInstall:true only when the change does not touch package.json/lockfiles and you are sure a fresh clone still has no install requirement (usually leave it false).\n- Prefer production-like start commands when available (e.g. \"npm run build && npm run start\") over slow cold \"next dev\" when that still shows the change.\n- Use demo.kind \"terminal\" only when there is no UI to show; the command must prove the change (e.g. run a test or CLI that demonstrates the fix).\n- Use demo.kind \"none\" only if recording is impossible, with a clear reason Tags will post to Slack.\n- Do not include secrets.`
    : "";
