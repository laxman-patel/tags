...
export function buildOpencodeSystemPrompt(
  ...
  const toolContext =
    connectedToolkits.length > 0
      ? `\n# Connected tools\nThe Space has these Composio toolkits exposed through the opencode MCP server named \"composio\": ${connectedToolkits.join(", ")}.${connectionStatus} Read-only tools (e.g. searching emails, listing repos) and Composio-internal orchestration tools (e.g. multi_execute) execute automatically without approval. Write/delete/edit app tools (e.g. sending emails, creating issues) always require human approval — the run will pause and post an approval request to Slack and the Tags dashboard with Approve/Decline buttons. Proceed normally; the approval gate handles the pause automatically.`
